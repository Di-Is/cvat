...
def _env_optional_flag(name: str) -> Optional[bool]:
    """
    Return True/False only when the env var is set to a meaningful value.

    This avoids overriding library defaults when the variable is unset or empty.
    """
    value = os.getenv(name)
    if value is None:
        return None
...
_SAM2_TRACKER_FAST_PREPROCESS = _env_flag_default("SAM2_TRACKER_FAST_PREPROCESS", True)
_SAM2_TRACKER_ALLOW_CPU_FALLBACK = _env_flag("SAM2_TRACKER_ALLOW_CPU_FALLBACK")
_SAM2_TRACKER_WARMUP_FRAMES = _env_int("SAM2_TRACKER_WARMUP_FRAMES", 2)
# Allow disabling triton cudagraphs via env for stability.
_USE_CUDAGRAPHS = _env_optional_flag("SAM2_TRACKER_USE_CUDAGRAPHS")
if _USE_CUDAGRAPHS is not None:
    # Only override torch default when explicitly requested.
    torch._inductor.config.triton.cudagraphs = _USE_CUDAGRAPHS
...
class _PerfLogger:
    ...
    @contextlib.contextmanager
    def phase(self, phase: str, *, allow_gpu_timing: bool = True, **extra: object):
        if not self._enabled:
            yield
            return
        ...
        record_gpu = self._supports_gpu_timing and allow_gpu_timing
        if record_gpu:
        ...
        try:
            ...
            if record_gpu and start_event is not None and end_event is not None:
                with torch.cuda.device(self._device):
                    end_event.record()
                try:
            ...
            payload.update(self._context.get())
            ...
            self._logger.info(
                "SAM2_TRACKER_LOG %s", json.dumps(payload, separators=(",", ":"))
            )
...
class _PreprocessedImage:
    ...
    preprocess_start_event: Optional[torch.cuda.Event] = None
    preprocess_end_event: Optional[torch.cuda.Event] = None
...
def _install_predictor_perf_hooks(
    ...
    def _wrap_method(
        ...
        attribute: str,
        phase: str,
        extra_factory: Optional[Callable[..., dict[str, object]]] = None,
    ) -> None:
    ...
    _wrap_method(
        ...
        extra_factory=lambda _current_feats, _feat_sizes, _pred_masks, _obj_logits, is_mask_from_pts: {
            "mask_from_points": is_mask_from_pts
        },
    )
    _wrap_method(
    ...
    setattr(predictor, "_cvat_perf_hooks_installed", True)
...
class _Sam2Tracker:
    def __init__(self, model_id: str, device: str = "cpu", **kwargs) -> None:
        ...
        predictor_cls = _CvatSAM2VideoPredictorVOS if self._vos_optimized else SAM2VideoPredictorVOS
    ...
    def preprocess_image(
        ...
        # Use a dedicated event pair on the preprocess stream when async to measure GPU time
        # without synchronizing the default stream.
        with self._perf_logger.phase("preprocess", allow_gpu_timing=not self._use_async_preprocess):
            preprocess_start_event: Optional[torch.cuda.Event] = None
            preprocess_end_event: Optional[torch.cuda.Event] = None
            ...
            if self._use_fast_preprocess:
                if self._use_async_preprocess and self._preprocess_stream is not None:
                    with torch.cuda.stream(self._preprocess_stream):
                        ...
                        preprocess_start_event.record(stream=self._preprocess_stream)
                ...
                if not _SAM2_TRACKER_ALLOW_CPU_FALLBACK:
                    raise RuntimeError(
                        "Fast preprocess is required; set SAM2_TRACKER_FAST_PREPROCESS=1 "
                        "or explicitly opt in to CPU fallback via SAM2_TRACKER_ALLOW_CPU_FALLBACK=1."
                    )
                ...
                if self._use_async_preprocess and self._preprocess_stream is not None:
                    with torch.cuda.stream(self._preprocess_stream):
                        preprocess_start_event = torch.cuda.Event(enable_timing=True)
                        preprocess_start_event.record(stream=self._preprocess_stream)
                        image_tensor = self._transform(image).unsqueeze(0).to(
            ...
            if self._use_async_preprocess and self._preprocess_stream is not None:
                with torch.cuda.stream(self._preprocess_stream):
                    ...
                    preprocess_end_event.record(stream=self._preprocess_stream)
            ...
            return _PreprocessedImage(
                ...
                preprocess_start_event=preprocess_start_event,
                preprocess_end_event=preprocess_end_event,
            )

    @torch.inference_mode()
    def cpu_preprocess_image(
        self, context: cvataa.TrackingFunctionContext, image: PIL.Image.Image
    ) -> tuple[torch.Tensor, tuple[int, int]]:
        """
        CPU-only preprocess: decode/resize/normalize on host, return a pinned tensor.
        Used by the double-buffer path to overlap CPU work with GPU forward_image.
        """
        with self._perf_logger.phase("preprocess_cpu"):
            rgb = image.convert("RGB")
            tensor = self._transform(rgb).unsqueeze(0).contiguous(memory_format=torch.channels_last)
            if self._use_async_preprocess:
                tensor = tensor.pin_memory()
            return tensor, (image.width, image.height)

    @torch.inference_mode()
    def forward_preprocessed_tensor(
        self, tensor: torch.Tensor, *, original_size: tuple[int, int]
    ) -> _PreprocessedImage:
        """
        Move a preprocessed CPU tensor to GPU and run SAM2 forward_image on the main thread.
        This keeps cudagraph TLS on the caller thread while still benefiting from non-blocking H2D.
        """
        ready_event: Optional[torch.cuda.Event] = None
        def _dynamo_disabled():
            return contextlib.nullcontext()

        with self._perf_logger.phase("preprocess"):
            with _dynamo_disabled():
                if self._use_async_preprocess and self._preprocess_stream is not None:
                    with torch.cuda.stream(self._preprocess_stream):
                        image_tensor = tensor.to(device=self._device, non_blocking=True)
                        backbone_out = self._predictor.forward_image(image_tensor)
                        ready_event = torch.cuda.Event()
                        ready_event.record(stream=self._preprocess_stream)
                else:
                    image_tensor = tensor.to(device=self._device, non_blocking=False)
                    backbone_out = self._predictor.forward_image(image_tensor)
    ...
    def _call_predictor(self, *, pp_image: _PreprocessedImage, frame_idx: int, **kwargs) -> dict:
        with self._perf_logger.scoped_context(frame_idx=frame_idx):
            ...
            preprocess_gpu_ms: Optional[float] = None
            if (
                pp_image.preprocess_start_event is not None
                and pp_image.preprocess_end_event is not None
            ):
                try:
                    # Wait on the preprocess stream event to make sure the timing is valid
                    torch.cuda.current_stream().wait_event(pp_image.preprocess_end_event)
                    preprocess_gpu_ms = pp_image.preprocess_start_event.elapsed_time(
                        pp_image.preprocess_end_event
                    )
                except Exception:
                    preprocess_gpu_ms = None

            with self._perf_logger.phase(
                "track_step",
                preprocess_gpu_ms=round(preprocess_gpu_ms, 3) if preprocess_gpu_ms is not None else None,
            ):
                if self._vos_optimized:
        ...
        with self._perf_logger.phase("track_postprocess", allow_gpu_timing=False):
            return {
    ...
    def track_batch(
        ...
        results: list[Optional[cvataa.TrackableShape]] = []
